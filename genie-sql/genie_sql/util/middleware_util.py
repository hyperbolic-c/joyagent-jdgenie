...
from genie_sql.model.context import RequestIdCtx
from genie_sql.util.log_util import AsyncTimer
...
class HTTPProcessTimeMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        async with AsyncTimer(key=f"{request.method} {request.url.path}") as timer_obj:
            response = await call_next(request)
            process_time = int((time.time() - timer_obj.start_time) * 1000)
